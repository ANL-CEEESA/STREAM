...
#
"""
    generate_data(demand_inc, elec_supply_inc, ccs_ccf, co2_reduction, fout_name)

Generates the excel file that contains input information for stre3am.

# Arguments
- `demand_inc::Float64`: Percentage/year increase of demand.
- `elec_supply_inc::Float64`: Percentage/year increase of electricity supply.
- `ccs_ccf::Float64`: CCS capture capital cost factor. 
- `co2_reduction::Float64`: CO_2 reduction w.r.t. to initial year (used for 
budget)
- `fout_name::String`: excel file output name, e.g. fout_name.xlsx

# Examples
```julia-repl
julia> generate_data(0.1, 0.25, 1.0, 0.5, "f1.xlsx")
1
```
"""
function generate_data(demand_inc, elec_supply_inc, ccs_ccf, co2_reduction, fout_name)
    # Generate the f0.xlsx input
    #
    # 80 ###########################################################################
    fac_f = "./samples/sample_cs_n=10_062025.csv"
    ...
    time_horizon = (n_periods * n_subperiods)* yr_subperiod
    ...
    # MMUSD/MT/yr (incumbent)
    loan_base_value = 50.0 * (sf_cash/sf_cap)
    fonm_base_value = loan_base_value * 2/100
    vonm_base_value = 3*1e-6 * (sf_cash/sf_cap) # 3USD->3e-06MMUSD

    d_fac = DataFrame(CSV.File(fac_f))
    # yearly capacity
    analysis_capacity_factor = 1
    analysis_co2_factor = 1
    d_fac[:, "CapTon"] = analysis_capacity_factor*d_fac[:, "CapTon"].*sf_cap  # scaled
    # heat intensity
    d_fac[:, "HiMBTUTon"] = d_fac[:, "HiMBTUTon"].*(sf_heat/sf_cap)  # scaled
    # elec intensity
    d_fac[:, "EiMBTUTon"] = d_fac[:, "EiMBTUTon"].*(sf_elec/sf_cap) # scaled
    # carbon intensity
    d_fac[:, "pETonTon"] = analysis_co2_factor*d_fac[:, "pETonTon"].*(sf_em/sf_cap)
    ...
    # ktonCO2/mmtu
    # we add a factor to reduce the intensity
    d_em[:, "CO2 Factor"] = d_em[:, "CO2 Factor"].*(1E-03)  # kg/mmbtu->ton/mmbtu
    ...
    d_growth = demand_inc #2/100  # percent
    agg_demand = agg_dem0 * (1 + d_growth)^(time_horizon - yr_subperiod)
    # this has - yr_period to adjust for where the last point should be
    ...
    # carbon capture not allowed in the previous states
    for l in loc_range
        state = d_fac[l, "State"]
    ...
    # 80 ###########################################################################
    # 5 electricity intensity mmbtu/kton
    ...
    # 80 #
    r_c_vOnm = ones(n_loc, n_rtft)

    usdToMmusd = 1e-6
    kg_1ToMt_1 = 1e3
    
    for k in 1:n_rtft
        r_c_vOnm[:, k] .= 3*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    end

    ### ### 2: Efficiency
    r_c_vOnm[:, 2] .= 2.75*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 3: Coal to NG (same as inc)
    # r_c_vOnm[:, 3] .= 
    ### ### 4: Full Electrification
    r_c_vOnm[:, 4] .+= 3*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 5: CCS
    r_c_vOnm[:, 5] .+= (8+2.2)*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 6: Alt patway
    r_c_vOnm[:, 6] .+= 2*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ...
    r_loanFact[:, :, :, 1] .= 0.0
    r_loanFact[:, :, :, 2] .= 10.0 * (sf_cash/sf_cap) # eff
    r_loanFact[:, :, :, 3] .= 10.0 * (sf_cash/sf_cap) # fs
    r_loanFact[:, :, :, 4] .= 100.0 * (sf_cash/sf_cap)  # elec
    r_loanFact[:, :, :, 5] .= 150.0 * (sf_cash/sf_cap)*ccs_ccf  # ccs
    r_loanFact[:, :, :, 6] .= 60.0 * (sf_cash/sf_cap)  # alt-pathway
    ...
    # 80 ###########################################################################
    # Fixed O and M
    r_c_fOnm = ones(n_loc, n_rtft)
    r_rhs_fOnm = zeros(n_loc, n_rtft) # 40
    for k in 1:n_rtft
        r_c_fOnm[:, k] .= r_loanFact[1,1,:,k]*2/100
    end
    # scaled!
    ...
    # calculating the maximum coefficients and rhs
    for n in 1:n_node
    ...
    n_c_U[:, 3, :] .*= 0.95
    ### ### 4: Full Electrification
    n_c_U[:, 4, 1] .+= n_c_H[:, 2, 1].*eToHfac[1] # taking the value of inc
    n_c_U[:, 4, 2] .+= n_c_H[:, 2, 2].*eToHfac[2]
    ### ### 5: CCS 
    ...
    # 80 ###########################################################################
    # 9 fixed/variable o and m 
    
    # variable O and M
    n_c_vOnm = ones(n_loc, n_new)
    ### ### 2: Incumbent 
    n_c_vOnm[:, 2] .= 3*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 3: BAT
    n_c_vOnm[:, 3] .= 2.75*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 4: Full Electrification (adds to inc)
    n_c_vOnm[:, 4] .+= 2*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 5: CCS (adds to inc)
    n_c_vOnm[:, 5] .+= (6+2.2)*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ### ### 6: alt-pathway (adds to inc)
    n_c_vOnm[:, 6] .+= 2*usdToMmusd*kg_1ToMt_1*(sf_cash/sf_cap)
    ...
    n_loanFact[:, 1] .= 0.0
    n_loanFact[:, 2] .= 50.0 * (sf_cash/sf_cap) # inc
    n_loanFact[:, 3] .= 60.0 * (sf_cash/sf_cap)  # eff
    n_loanFact[:, 4] .= 100.0 * (sf_cash/sf_cap)  # elec
    n_loanFact[:, 5] .= 150.0 * (sf_cash/sf_cap)*ccs_ccf  # ccs
    n_loanFact[:, 6] .= 60.0 * (sf_cash/sf_cap)  # alt-pathway

    # 80 ###########################################################################

    #n_c_fOnm = ones(n_loc, n_new) # 80
    n_c_fOnm = ones(n_loc, n_new)
    # two percent of capital
    for k in 1:n_new
        n_c_fOnm[:, k] .= n_loanFact[:, k] * 2/100
    end
    ...
    for l in loc_range
        for i in 1:n_periods
            for j in 1:n_subperiods
                ...
                c_r_ehf_cost[i, j, l, 1] = d_fuel_c[9, 22]*musdTousd*(1+coal_inc)^y
                c_r_ehf_cost[i, j, l, 2] = d_fuel_c[2, 18]*musdTousd*(1+ng_inc)^y
                c_r_ehf_cost[i, j, l, 3] = d_fuel_c[7, 20]*musdTousd*(1+ng_inc)^y
                #
                c_n_ehf_cost[i, j, l, 1] = d_fuel_c[9, 22]*musdTousd*(1+coal_inc)^y
                c_n_ehf_cost[i, j, l, 2] = d_fuel_c[2, 18]*musdTousd*(1+ng_inc)^y
                c_n_ehf_cost[i, j, l, 3] = d_fuel_c[7, 20]*musdTousd*(1+ng_inc)^y
            end
        end
    end
    ...
    c_cts_cost = ones(n_loc) * 1e-06 * 10.0
    # musd/tonneCO2
    ...
    #
    #co2_reduction = 0.5
    co2_endpoint = co2_total0 * (1 - co2_reduction) # 

    co2_slope = (co2_endpoint - co2_total0)/(time_horizon)
    ...
    co2_budget .= (co2_total0 + co2_endpoint)*(1/2)*(time_horizon) # trapezoid

    @info "CO2 start =\t$(co2_total0)"
    @info "CO2 end =\t$(co2_endpoint)"
    @info "CO2 reduction =\t$(co2_reduction)"
    @info "CO2 budget =\t$(co2_budget)"

    # minimum capacity
    min_cpr = ones(n_loc) .* 0.50

    # initial total supply of electricity

    # 
    t_elec_0 = 0.0
    for n in 1:n_node
        # electricity mmbtu
        t_elec_0 += sum((1.0.-r_c_UonSite[1, 1, :, 1, n]).*r_c_U[:, 1, n].*cp0n[:, n])
    end


    elec_supply_proj = zeros(n_periods, n_subperiods)
    for i in 1:n_periods
        for j in 1:n_subperiods
            y = (i-1)*yr_period + (j-1)*yr_subperiod
            elec_supply_proj[i, j] = t_elec_0*(1+elec_supply_inc)^y
        end
    end
